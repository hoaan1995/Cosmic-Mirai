...
func (this *Admin) Handle() {
	...
	go func() {
		...
		for {
			...
			if _, err := this.conn.Write([]byte(fmt.Sprintf("\033]0; Cosmic Net > Bots: [%d] | User: [%s] | Methods:[5] | Bypass:[1]\007", BotCount, username))); err != nil {
				this.conn.Close()
				break
			}
			i++
    ...
    this.conn.Write([]byte("\r\n"))
    this.conn.Write([]byte("\x1b[1;33mWelcome to cosmic\r\n"))
    this.conn.Write([]byte("\r\n"))
    this.conn.Write([]byte("\r\n"))
